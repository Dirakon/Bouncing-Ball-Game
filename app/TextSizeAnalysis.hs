...
estimateTextSize [] = (0, fontHeight)
estimateTextSize (character : others) =
  (fromMaybe defaultCharacterSize (lookup character characterSizes) + fst (estimateTextSize others), fontHeight)
...
alignedCenterText text = translate (- textWidth * 0.5) (- textHeight * 0.5) (Text text)
  where
    (textWidth, textHeight) = estimateTextSize text

-- | Update character sizes for characterSizes. 
